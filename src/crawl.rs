...
impl DataSource for Crawler {
    fn fetch(
        ...
        let base_url = &self.site_config.base_url;
        let categories = &self.site_config.categories;
        let extensions = &self.attachment_extensions;
        let selectors = &self.site_config.selectors;
        let crawler_config = &self.crawler_config;
        ...
        for category in categories {
            while !end_reached_map[category] {
                ...
                let status = Self::fetch_pages(
                    ...
                    date_after,
                    with_contents_only,
                    crawler_config.keep_complex_tables,
                )?;
...
impl Crawler {
    ...
    fn fetch_content(
        ...
        if let Some(content_element) = document.select(&content_sel).next() {
            let plain_text = get_pretty_text(content_element, &base_url, keep_complex_tables);
